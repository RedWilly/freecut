...
export function planLayoutVideoSync(input: {
  ...
  return {
    shouldPause: false,
    seekTo: !input.isPlaying && Math.abs(input.currentTime - input.targetTime) > 0.016
      ? input.targetTime
      : null,
    shouldMarkInitialSyncComplete: false,
  };
...
export function planPlayingVideoInitialSync(input: {
  ...
  return {
    seekTo: Math.abs(input.currentTime - input.targetTime) > 0.016
      ? input.targetTime
      : null,
    shouldMarkInitialSyncComplete: true,
...
export function planPlayingVideoDriftCorrection(input: {
  ...
  return {
    shouldPause: false,
    seekTo: (videoFarAhead || (videoBehind && timeSinceLastSync > 80))
      ? input.targetTime
      : null,
  };
}
...
export function planPausedVideoFrameSync(input: {
  ...
  return {
    shouldPause: false,
    seekTo: input.frameChanged
      && input.canSeek
      && Math.abs(input.currentTime - input.targetTime) > 0.016
      ? input.targetTime
      : null,
  };
}
